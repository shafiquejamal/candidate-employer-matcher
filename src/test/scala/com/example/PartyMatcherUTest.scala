...
class PartyMatcherUTest extends FlatSpecLike with ShouldMatchers {
  ...
  "(no duplicated preferences, etc)" in {
    ...
    val expected = Seq(
      Candidate(1,Vector(1, 3, 2),Some(Employer(3,Vector(1, 2),1))),
      Candidate(2,Vector(1, 2, 3),Some(Employer(1,Vector(2, 1),1))),
      Candidate(3,Vector(2, 3, 1),Some(Employer(2,Vector(3, 1),1)))
    )
